...
class MoveToGoal(Node):
    def __init__(self, args=None):
        ...
        self.timer = self.create_timer(0.05, self.timer_callback)    # Timer -> for 0.1 secs, it will keep calling timer_callback
    ...
    def timer_callback(self):
        ...
        if not self.direction_found_bool:
            msg = Twist()
            self.get_direction('odom', 'base_link', 'base_laser_front_link')   # Transforming whatever goal is perceived  
            # wrt laser scanner to base link 
    ...
    def get_direction(self, odom_frame, base_link_frame, base_laser_frame):
        ...
        try:
            ...
            """
            ### YOUR CODE HERE ###
            # Getting goal_wrt_odom
            # transform_odom_wrt_base_link = self.get_transform(base_link_frame, odom_frame)
            # self.goal_wrt_odom = self.tf_buffer.lookup_transform(self.goal_wrt_base_link, transform_odom_wrt_base_link, rclpy.time.Time(), rclpy.time.Duration(seconds=1.0))

            # Set desired velocity
            self.x_vel = self.goal_wrt_odom.pose.position.x
            self.y_vel = self.goal_wrt_odom.pose.position.y
            self.goal_wrt_base_link.pose.position.x = transform_laser_wrt_odom.translation.x + self.goal_wrt_base_laser.pose.position.x
            self.goal_wrt_base_link.pose.position.y = transform_laser_wrt_odom.translation.y + self.goal_wrt_base_laser.pose.position.y
    ...
    def get_distance(self, target_frame, source_frame): # L2 norm of the given points, we are checking if frames are overlapping or not
        """
